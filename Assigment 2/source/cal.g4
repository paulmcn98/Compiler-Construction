...
statement:                ID ASSIGN expression SEMI                         # AssignStm
                         ...
                         |BEGIN statement_block END                         # StmblkStm
                         |IF condition BEGIN statement_block END
                         (ELSE BEGIN statement_block END)?                     # IfStm
...
expression:		          frag binary_arith_op frag                         # AddSubOp
                         |LBR expression RBR                                # Parens
                         |ID LBR arg_list RBR                               # ArgList
                         |frag                                              # FragExp
                         ;

binary_arith_op:         op = (ADD | MINUS);

frag:                    ID | MINUS ID | NUMBER | BV;

condition:               NEG condition | LBR condition RBR | expression comp_op expression | condition op = (OR | AND) condition;

comp_op:                 op = (EQUAL | NOTEQUAL | LT | LTE | GT | GTE);
